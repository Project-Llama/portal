...
try:
    stream = ollama.chat(
        ...
        messages=[
            {
                "role": "system",
                "content": open("system.txt", "r").read() + f" You are converting code from {args.from_lang} to {args.to_lang}. You will be as accurate as possible.",
            },
            {"role": "user", "content": f"Code to convert:\n```\n{content}\n```"},
        ],
        stream=True,
    )
    with open(f'outputs/output_{args.file_path}{file_extension}', 'w') as output_file:
        for chunk in stream:
    ...
    print(f"Translation complete! Check the outputs/output{file_extension} file for the translated code.")
